# Install Raven Local on Windows

Raven Local ships as a `.msi` for x64 Windows. Microsoft's SmartScreen Defender flags new MSIs from unverified publishers — the binary isn't signed with a Microsoft-trusted certificate yet, so first launch shows the *"Windows protected your PC"* dialog.
...
1. Download the `.msi` from the [latest release](https://github.com/ravencloak-org/Raven/releases?q=raven-local).
...
6. Launch *Raven Local* from the Start Menu.
...
**Settings → Apps → Installed apps → Raven Local → Uninstall.**
...
```powershell
docker volume rm $(docker volume ls -q --filter "label=com.docker.compose.project=raven-local")
```
...
- **App opens but compose doesn't come up** — Docker Desktop for Windows isn't running. Start it from the Start Menu, then re-launch Raven Local.
